...
const MeetCard = ({ user, peer }) => {
    ...
    return (
        <Box sx={styles.MeetCard}>
            <video playsInline autoPlay controls={false} ref={videoRef} className="object-cover rounded-lg" style={{
                width: styles.MeetCard.width,
                height:styles.MeetCard.height
            }} />
            {isActive && <Box sx={{ width: "100%", height: "100%", position: 'absolute', backgroundColor: "#303030", ...styles.singleCenter }}>
            ...
            <Typography variant="body1" className="absolute bottom-4 left-4" >{user?.name}</Typography>
        </Box>
    )
}
